...
const GAME_CATEGORIES = [
  { id: 'memory', emoji: '🧠', title: 'Memory Match', desc: 'Match words to pictures!', color: '#4A90E2', glowClass: 'ls-cat-glow-blue' },
  { id: 'letter', emoji: '🅰️', title: 'Letter Match', desc: 'Match big and small letters!', color: '#F39C12', glowClass: 'ls-cat-glow-amber' },
  { id: 'sound', emoji: '🎵', title: 'Sound Match', desc: 'Listen and pick the right letter!', color: '#27AE60', glowClass: 'ls-cat-glow-green' },
  { id: 'builder', emoji: '🏗️', title: 'Word Builder', desc: 'Fix the scrambled words!', color: '#8E44AD', glowClass: 'ls-cat-glow-purple' },
  { id: 'missing', emoji: '❓', title: 'Missing Letter', desc: 'Fill in the missing letter!', color: '#E74C3C', glowClass: 'ls-cat-glow-red' },
  { id: 'quick', emoji: '⚡', title: 'Quick Tap', desc: 'Tap the matching picture fast!', color: '#00BCD4', glowClass: 'ls-cat-glow-cyan' }
];

export default function Games({ speakText, speechSpeed, setSpeechSpeed, replayAudio, onBack }) {
  const [activeGame, setActiveGame] = useState(null);
  const [completedGames, setCompletedGames] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('dyslexia_games_scores')) || [];
    } catch { return []; }
  });
  ...
  const handleGameComplete = () => {
    if (activeGame && !completedGames.includes(activeGame)) {
      ...
      localStorage.setItem('dyslexia_games_scores', JSON.stringify(newCompleted));
    ...
    return (
      <div className="ls-root">
        <div className="ls-page-header">
          ...
          <div className="ls-page-title">🎮 Choose a Game</div>
          <div className="ls-page-sub">Have fun and earn shiny badges!</div>
        </div>
        ...
        <div className="ls-cat-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', padding: '1rem', paddingBottom: '3rem' }}>
          {GAME_CATEGORIES.map(game => (
            ...
            >
              ...
              <div className="ls-cat-cta" style={{ color: game.color }}>Play Now →</div>
            </button>
          ))}
        </div>
      </div>
